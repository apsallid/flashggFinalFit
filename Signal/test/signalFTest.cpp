  ...
  if(verbose_) std::cout << "[INFO] on cat " << flashggCats_[cat] <<  " start looping through procs  " << procs.size() << " to get datasets " <<std::endl;
    // now main loop through processes...
      ...
      // access dataset and immediately reduce it!
			if (isFlashgg_){
			        std::string thecurcat = flashggCats_[cat];
				std::size_t foundcat = thecurcat.find_last_of('_');
				int bdtregion = 0;
				RooDataSet *data0;
				//Check if we are on the category we wish to optimize
				if ( flashggCatsIn_[0] == thecurcat.substr(0,foundcat)   ){
				  bdtregion = std::stoi( thecurcat.substr(foundcat+1) );
				  RooDataSet *dataFullin   = (RooDataSet*)inWS->data( Form("%d%s",mass_,proc.c_str()),
          Form("%s_%d_13TeV_%s",proc.c_str(),mass_,flashggCatsIn_[0].c_str()));

				  data0 =  reduceDataset(dataFullin, Form("%s_%d_13TeV_%s",proc.c_str(),mass_,flashggCats_[cat].c_str()) , cutforBDT_[bdtregion], cutforBDT_[bdtregion+1]);				  
				} else {
				  data0 = (RooDataSet*)inWS->data( Form("%d%s",mass_,proc.c_str()), Form("%s_%d_13TeV_%s",proc.c_str(),mass_,flashggCats_[cat].c_str()));
				}
