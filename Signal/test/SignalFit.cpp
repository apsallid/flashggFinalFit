...
RooDataSet * reduceDataset(RooDataSet *data0){
  ...
  for (int i=0 ; i < data0->numEntries() ; i++){
    ...
    data->add( RooArgList(*mass_, *dijet_mva_, *dZ_, *weight0), weight0->getVal() );
  }
  return data;
}
...
int main(int argc, char *argv[]){
  ...
  mass_->setUnit("GeV");
  dijet_mva_ = (RooRealVar*)inWS->var("dijet_mva");
  dijet_mva_->SetTitle("dijet_mva");
  ...
  if( mass_ && dijet_mva_ && dZ_ && intLumi_){
    if (verbose_) std::cout << "[INFO] RooRealVars mass, dijet_mva, intL and dZ, found ? " << mass_ << ", " << dijet_mva_ << ", " << intLumi_  << ", "<< dZ_<<  std::endl;
  } else {
    std::cout << "[ERROR] could not find some of these RooRealVars in WS: mass_ " << mass_ << ", " << dijet_mva_ << " dZ " << dZ_ << " intLumi " << intLumi_<< ".  exit(1) "<< std::endl;
    exit(1);
  }
  ...
  while (datfile.good()){
    ...
    // the default info need: proc, tag, nRv, nrWV
    string proc = els[0];
    ...
    // have a different approach now but could re-use machinery.
  ...
  // now start the proper loop, so loop over the maps we filled above.
  for (unsigned int iLine = 0 ; iLine < map_proc_.size() ; iLine++){
    ...
    map<int,RooDataSet*> datasetsBDTG;
    ...
    for (int mh=mhLow_; mh<=mhHigh_; mh+=5){
      ...
      if (verbose_)std::cout << "[INFO] Opening dataset called "<< Form("%s_%d_13TeV_%s",proc.c_str(),mh,cat.c_str()) << " in WS " << inWS << std::endl;
	...
	data0Ref   = rvwvDataset(intLumiReweigh(reduceDataset(		      
	...
	dataRVRef=(RooDataSet*) dataRV->Clone();
      ...
      // if there are few actual entries or if there is an  overall negative sum of weights...
      // or if it was specified that one should use the replacement dataset, then need to replace!
    ...
    // right vertex
    if (verbose_) std::cout << "[INFO] preparaing initialfit RV" << std::endl;
    ...
    //--------------------------------------------------------------------------------------
    //This for the BDT fitting
    if (verbose_) std::cout << "[INFO] preparing initialfit for BDT" << std::endl;
    ...
    if (!cloneFits_) {
      if (verbose_) std::cout << "[INFO] BDT building pdf " << std::endl;
      initFitBDT.buildBDTpdf(Form("%s_cat%s_BDT",proc.c_str(),cat.c_str()));
      if (verbose_) std::cout << "[INFO] BDT setting datasets in initial FIT " << std::endl;
