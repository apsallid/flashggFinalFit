...
int main(int argc, char *argv[]){
  ...
  if (verbose_){
    ...
    for( unsigned int i =0 ; i<flashggCats_.size() ; i++){
  ...
  for (unsigned int ifile =0; ifile < filename_.size() ; ifile++){
    ...
    std::cout << "[INFO] progress " << progress <<std::endl;
	...
	if (isSig) {
	  RooFitResult *fitTestGaus;
	  fitTestGaus = gauss->fitTo(*dataPlot,RooFit::Save(1),RooFit::Verbose(0),RooFit::SumW2Error(kTRUE), RooFit::Minimizer("Minuit2","minimize")); //FIXME
	  float gausnll = fitTestGaus->minNll();
	  std::cout << "[INFO] gausnll " << gausnll << std::endl;
	  ...
	  fitTest = p2->fitTo(*dataPlot,RooFit::Save(1),RooFit::Verbose(0),RooFit::SumW2Error(kTRUE), RooFit::Minimizer("Minuit2","minimize")); //FIXME
	  float bernsnll = fitTest->minNll();
	  std::cout << "[INFO] bernsnll " << bernsnll << std::endl;
