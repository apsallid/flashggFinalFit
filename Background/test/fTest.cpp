...
int main(int argc, char* argv[]){
  ...
  FILE *resFile, *resFile_BDT ;
  if  (singleCategory >-1) {
    resFile = fopen(Form("%s/fTestResults_%s.txt",outDir.c_str(),flashggCats_[singleCategory].c_str()),"w");
    resFile_BDT = fopen(Form("%s/fTestResults_bdt_%s.txt",outDir.c_str(),flashggCats_[singleCategory].c_str()),"w");
  } else { 
    resFile = fopen(Form("%s/fTestResults.txt",outDir.c_str()),"w");
    resFile_BDT = fopen(Form("%s/fTestResultsBDT.txt",outDir.c_str()),"w");
  }
  vector<map<string,int> > choices_vec, choices_vec_BDT;
  vector<map<string,std::vector<int> > > choices_envelope_vec, choices_envelope_vec_BDT;
  vector<map<string,RooAbsPdf*> > pdfs_vec, pdfs_vec_BDT;
  ...
  for (int cat=startingCategory; cat<ncats; cat++){
    ...
    map<string,int> choices, choices_BDT;
    map<string,std::vector<int> > choices_envelope, choices_envelope_BDT;
    map<string,RooAbsPdf*> pdfs, pdfs_BDT;
    map<string,RooAbsPdf*> allPdfs, allPdfs_BDT;
    string catname;
    if (isFlashgg_){
    ...
    data = (RooDataSet*)&thisdataBinned;
    RooDataHist thisdataBinned_BDT(thisdataBinned_name_bdt.c_str(),"data",*BDT,*dataFull);
    data_BDT = (RooDataSet*)&thisdataBinned_BDT;

    RooArgList storedPdfs("store");
    RooArgList storedPdfs_BDT("store");

    fprintf(resFile,"\\multicolumn{4}{|c|}{\\textbf{Category %d}} \\\\\n",cat);
    fprintf(resFile,"\\hline\n");
    fprintf(resFile_BDT,"\\multicolumn{4}{|c|}{\\textbf{Category %d}} \\\\\n",cat);
    fprintf(resFile_BDT,"\\hline\n");
	...
	// RooAbsPdf *bkgPdf_BDT = getPdf(pdfsModel_BDT,*funcType,order,Form("ftest_pdf_bdt_%d_%s",cat,ext.c_str()));
	if (!bkgPdf // || !bkgPdf_BDT
	    ){
    ...
    //========================================================================================
    //For the bdt now
    MinimimNLLSoFar=1e10;
    int simplebestFitPdfIndex_BDT = 0;
    // Standard F-Test to find the truth functions for the bdt now
    for (vector<string>::iterator funcType=functionClasses.begin(); 
	 funcType!=functionClasses.end(); funcType++){
      ...
      RooAbsPdf *prev_pdf=NULL;
      RooAbsPdf *cache_pdf=NULL;
      std::vector<int> pdforders_BDT;

      int counter =0;
      //	while (prob<0.05){
      while (prob<0.05 && order < 7){ //FIXME
	RooAbsPdf *bkgPdf = getPdf(pdfsModel_BDT,*funcType,order,Form("ftest_pdf_bdt_%d_%s",cat,ext.c_str()));
	if (!bkgPdf){
	  // assume this order is not allowed
	  order++;
	}
	  ...
	  int fitStatus = 0;
	  //thisNll = fitRes->minNll();
	  runFit(bkgPdf,data_BDT,&thisNll,&fitStatus,/*max iterations*/3);//bkgPdf->fitTo(*data,Save(true),RooFit::Minimizer("Minuit2","minimize"));
	  if (fitStatus!=0) std::cout << "[WARNING] Warning -- Fit status for " << bkgPdf->GetName() << " at " << fitStatus <<std::endl;
	  chi2 = 2.*(prevNll-thisNll);
	  if (chi2<0. && order>1) chi2=0.;
	  if (prev_pdf!=NULL){
	    prob = getProbabilityFtest(chi2,order-prev_order,prev_pdf,bkgPdf,BDT,data_BDT
				       ,Form("%s/Ftest_BDT_from_%s%d_cat%d.pdf",outDir.c_str(),funcType->c_str(),order,cat));
	    std::cout << "[INFO]  F-test BDT Prob(chi2>chi2(data)) == " << prob << std::endl;
	  } else {
	    prob = 0;
	  }
	  double gofProb=0;
	  // otherwise we get it later ...
	  if (!saveMultiPdf) plot(BDT,bkgPdf,data_BDT,Form("%s/BDT%s%d_cat%d.pdf",outDir.c_str(),funcType->c_str(),order,cat),flashggCats_,fitStatus,&gofProb);
	  cout << "[INFO]\t " << *funcType << " " << order << " " << prevNll << " " << thisNll << " " << chi2 << " " << prob << endl;
	  //fprintf(resFile,"%15s && %d && %10.2f && %10.2f && %10.2f \\\\\n",funcType->c_str(),order,thisNll,chi2,prob);
	  prevNll=thisNll;
	  cache_order=prev_order;
	...
	counter++;
      ...
      fprintf(resFile_BDT,"%15s & %d & %5.2f & %5.2f \\\\\n",funcType->c_str(),cache_order+1,chi2,prob);
      choices_BDT.insert(pair<string,int>(*funcType,cache_order));
      pdfs_BDT.insert(pair<string,RooAbsPdf*>(Form("%s%d",funcType->c_str(),cache_order),cache_pdf));
	...
	while (prob<upperEnvThreshold){
	  RooAbsPdf *bkgPdf = getPdf(pdfsModel_BDT,*funcType,order,Form("env_bdt_pdf_%d_%s",cat,ext.c_str()));
	  if (!bkgPdf ){
	    // assume this order is not allowed
	    if (order >6) { std::cout << " [WARNING] could not add ] " << std::endl; break ;}
	    order++;
	    ...
	    // Calculate goodness of fit for the thing to be included (will use toys for lowstats)!
	    double gofProb =0; 
	    plot(BDT,bkgPdf,data_BDT,Form("%s/bdt_%s%d_cat%d.pdf",outDir.c_str(),funcType->c_str(),order,cat),flashggCats_,fitStatus,&gofProb);
		...
		std::cout << "[INFO] Adding to Envelope " << bkgPdf->GetName() << " "<< gofProb 
			  << " 2xNLL + c is " << myNll + bkgPdf->getVariables()->getSize() <<  std::endl;
		allPdfs_BDT.insert(pair<string,RooAbsPdf*>(Form("%s%d",funcType->c_str(),order),bkgPdf));
		storedPdfs_BDT.add(*bkgPdf);
		pdforders_BDT.push_back(order);

		// Keep track but we shall redo this later
		if ((myNll + bkgPdf->getVariables()->getSize()) < MinimimNLLSoFar) {
		  simplebestFitPdfIndex_BDT = storedPdfs_BDT.getSize()-1;
		  MinimimNLLSoFar = myNll + bkgPdf->getVariables()->getSize();
		}
	      }
	    }
	...
	fprintf(resFile_BDT,"%15s & %d & %5.2f & %5.2f \\\\\n",funcType->c_str(),cache_order+1,chi2,prob);
	choices_envelope_BDT.insert(pair<string,std::vector<int> >(*funcType,pdforders_BDT));
      }
    }

    fprintf(resFile_BDT,"\\hline\n");
    choices_vec_BDT.push_back(choices_BDT);
    choices_envelope_vec_BDT.push_back(choices_envelope_BDT);
    pdfs_vec_BDT.push_back(pdfs_BDT);
    
    plot(BDT,pdfs_BDT,data_BDT,Form("%s/bdt_truths_cat%d",outDir.c_str(),cat),flashggCats_,cat);
	...
	catname = Form("cat%d",cat);
      ...
      RooCategory catIndex_BDT(catindexname.c_str(),"c");
      RooMultiPdf *pdf = new RooMultiPdf(Form("CMS_hgg_%s_%s_bkgshape",catname.c_str(),ext.c_str()),"all pdfs",catIndex,storedPdfs);
      RooMultiPdf *pdf_BDT = new RooMultiPdf(Form("CMS_hgg_bdt_%s_%s_bkgshape",catname.c_str(),ext.c_str()),"all pdfs",catIndex_BDT,storedPdfs_BDT);
      RooRealVar nBackground(Form("CMS_hgg_%s_%s_bkgshape_norm",catname.c_str(),ext.c_str()),"nbkg",data->sumEntries(),0,10E8);
      RooRealVar nBackground_BDT(Form("CMS_hgg_bdt_%s_%s_bkgshape_norm",catname.c_str(),ext.c_str()),"nbkg",data_BDT->sumEntries(),0,10E8);
      ...
      int bestFitPdfIndex_BDT = getBestFitFunction(pdf_BDT,data_BDT,&catIndex_BDT,!verbose);
      catIndex.setIndex(bestFitPdfIndex);
      catIndex_BDT.setIndex(bestFitPdfIndex_BDT);
      std::cout << "// ------------------------------------------------------------------------- //" <<std::endl; 
      ...
      std::cout << "// ------------------------------------------------------------------------- //" <<std::endl; 
      std::cout << "[INFO] Created MultiPdf " << pdf_BDT->GetName() << ", in Category " << cat << " with a total of " << catIndex_BDT.numTypes() << " pdfs"<< std::endl;
      storedPdfs_BDT.Print();
      std::cout << "[INFO] Best Fit Pdf = " << bestFitPdfIndex_BDT << ", " << storedPdfs_BDT.at(bestFitPdfIndex_BDT)->GetName() << std::endl;
      std::cout << "// ------------------------------------------------------------------------- //" <<std::endl;
      std::cout << "[INFO] Simple check of index "<< simplebestFitPdfIndex_BDT <<std::endl;

      mass->setBins(nBinsForMass);
      BDT->setBins(nBinsForBDT);
      RooDataHist dataBinned(Form("roohist_data_mass_%s",catname.c_str()),"data",*mass,*dataFull);
      RooDataHist dataBinned_BDT(Form("roohist_data_bdt_%s",catname.c_str()),"data",*BDT,*dataFull);
      ...
      outputws->import(*pdf_BDT);
      outputws->import(nBackground_BDT);
      outputws->import(catIndex_BDT);
      outputws->import(dataBinned_BDT);
      outputws->import(*data_BDT);
      plot(BDT,pdf_BDT,&catIndex_BDT,data_BDT,Form("%s/multipdf_bdt_%s",outDir.c_str(),catname.c_str()),flashggCats_,cat,bestFitPdfIndex_BDT);
  ...
  cout << "[RESULT] Recommended options for mass " << endl;
  ...
  cout << "[RESULT] Recommended options for BDT " << endl;

  for (int cat=startingCategory; cat<ncats; cat++){
    cout << "Cat " << cat << endl;
    fprintf(dfile,"cat=%d\n",cat); 
    for (map<string,int>::iterator it=choices_vec_BDT[cat-startingCategory].begin(); it!=choices_vec_BDT[cat-startingCategory].end(); it++){
      cout << "\t" << it->first << " - " << it->second << endl;
      fprintf(dfile,"truth=%s:%d:%s%d\n",it->first.c_str(),it->second,namingMap[it->first].c_str(),it->second);
    }
    for (map<string,std::vector<int> >::iterator it=choices_envelope_vec_BDT[cat-startingCategory].begin(); it!=choices_envelope_vec_BDT[cat-startingCategory].end(); it++){
      std::vector<int> ords = it->second;
      for (std::vector<int>::iterator ordit=ords.begin(); ordit!=ords.end(); ordit++){
	fprintf(dfile,"paul=%s:%d:%s%d\n",it->first.c_str(),*ordit,namingMap[it->first].c_str(),*ordit);
      }
